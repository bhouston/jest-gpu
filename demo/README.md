...
To use this as a project outside this repository, run these commands from the repository root.
They export only tracked demo files, so local `node_modules` links and build artifacts are not
copied, then replace the two `workspace:*` versions with published versions:

```sh
mkdir ../my-gpu-tests
git archive HEAD:demo | tar -x -C ../my-gpu-tests
cd ../my-gpu-tests
npm pkg set devDependencies.jest-environment-webgl-node=latest
npm pkg set devDependencies.jest-environment-webgpu-node=latest
pnpm install
pnpm approve-builds
pnpm test
```

When `pnpm approve-builds` prompts, approve the native GPU and transform dependencies. pnpm blocks
dependency build scripts by default outside this repository, and those scripts install the native
WebGL, WebGPU, and SWC binaries used by the tests.
